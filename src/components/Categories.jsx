    ...
    <div className="categories">
      <ul>
        ...
        && items.map((categoryName, index) => (
          <li className={activeItem === index ? 'active' : ''} onClick={() => onSelectItem(index)} key={`${categoryName}_${index}`}>{categoryName}</li>
        )
        )}
      </ul>
    </div>
...
// Если мы рендерим элементы с мпощью map
...
// Хуки setState, сейчас при нажатии на категорию вызовается onClick 
